...
export default function ContactForm({ productName="" }: { productName?: string }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("https://formspree.io/f/xbdwgkro", {
        ...
        body: JSON.stringify({ name:form.name, phone:form.phone, product:productName, message:form.message, _subject:`Báo giá từ ${form.name} – ${form.phone}` }),
      });
      if (!res.ok) throw new Error();
      setStatus("done"); setForm({ name:"", phone:"", message:"" }); setErrors({});
  ...
  if (status==="done") return (
    <div className="bg-green-4 border border-green/20 rounded-xl p-8 max-w-xl text-center">
      ...
      <p className="text-sm text-gray-600">Toàn Phát sẽ liên hệ sớm. Hotline: <a href="tel:0911472286" className="text-green font-semibold">0911.472.286</a></p>
      <button onClick={()=>setStatus("idle")} className="mt-4 text-xs text-green font-semibold hover:underline">Gửi yêu cầu khác</button>
    </div>
  );
  return (
    <form onSubmit={handleSubmit} className="bg-gray-50 rounded-xl p-6 max-w-xl" noValidate>
      <h3 className="font-bold text-lg mb-1">{productName ? "Yêu Cầu Báo Giá" : "Gửi Yêu Cầu Liên Hệ"}</h3>
      {productName && <p className="text-xs text-gray-500 mb-4">Sản phẩm: <span className="font-semibold text-green">{productName}</span></p>}
      <div className="space-y-3">
        <div>
          <input placeholder="Họ và tên *" value={form.name} onChange={e=>{setForm({...form,name:e.target.value});setErrors({...errors,name:undefined})}}
        ...
        <div>
          <input placeholder="Số điện thoại *" value={form.phone} type="tel" inputMode="numeric"
        ...
        <textarea placeholder="Ghi chú (số lượng, yêu cầu...)" value={form.message} rows={3}
        ...
        <p className="text-[11px] text-gray-400 text-center">Hoặc gọi: <a href="tel:0911472286" className="text-green font-semibold">0911.472.286</a></p>
      </div>
    </form>
  );
}
